...
function getForecastItemElement(items, location) {
  return items.find((item) => {
    ...
    return (
      title &&
      title.toLowerCase().includes("forecast") &&
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    if (headers.length > 0) {
      for (let headerIndex = 0; headerIndex < headers.length; headerIndex++) {
        ...
        if (headerValue.toLowerCase().includes("shore")) {
          ...
          headerValue.toLowerCase().includes("day") ||
          headerValue.toLowerCase().includes("night")
        ) {
          days.push(headerValue);
        } else if (
          headerValue.toLowerCase().includes("am") ||
          headerValue.toLowerCase().includes("pm")
        ) {
    ...
    for (let cellIndex = 0; cellIndex < cells.length; cellIndex++) {
      ...
      if (cellValue.toLowerCase().includes("facing")) {
        ...
        const totalValue = heightValues.reduce(
